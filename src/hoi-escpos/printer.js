...
Printer.prototype.setColor = function(color){
  var header = '\x1b\x72' + color;
  this.buffer.write(header);
  return this;
}

Printer.prototype.setColor2 = function(color){
  var header = '\x1b\x72\x00';
  this.buffer.write(header);
  return this;
}
 ...
 * @return {[type]}       [description]
 */
Printer.prototype.hoiImage = function(image, density){
  if(!(image instanceof Image))
    throw new TypeError('Only escpos.Image supported');
  density = density || 'd24';
  var n = 1;
  var header = _.BITMAP_FORMAT['BITMAP_S8'];
  var bitmap = image.toBitmap(1 * 8);
  var self = this;
  this.lineSpace(16); // set line spacing to 0
  bitmap.data.forEach(function (line) {
    self.buffer.write(header);
    self.buffer.writeUInt16LE(line.length / n);
    self.buffer.write(line);
